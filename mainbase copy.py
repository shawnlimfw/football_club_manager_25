...
def game_setup():
    ...
    def game_setup_firstpage():
        global team_name
        print('')
        print('Welcome to Football Club Manager 2025!')
        ...
        for key, value in tactics.items():
            if value == '':
                ...
                if value['Injured'] != False:
                    eligible = False
        if eligible == True:  #FOR TESTING: CHANGE TO TRUE (FALSE to advance with empty lineup)
            matchday_game()
        if eligible == False:
    ...
    def matchday_game():
        ...
        def stage_events():
            ...
            for minute in temp_events:
                ...
                p = random.random()
                if p < 0.00050: #TESTING: CHANGED FROM 0.00075 TO 0.00050 (code: dj83974dhduie891)
                    card_events[minute] = (team, 'Red Card')
                    temp_events.remove(minute)
                elif p < 0.018: #TESTING: CHANGED FROM 0.024 TO 0.018 (code: hde23iuy3786573f)
                    card_events[minute] = (team, 'Yellow Card')
                    temp_events.remove(minute)
            for minute in temp_events:
                team = random.choice([team_name, opponent])
                p = random.random()
                if p < 0.0015: #TESTING: CHANGED FROM 0.003 TO 0.0015 (code: ytd7674t76duh0jd)
